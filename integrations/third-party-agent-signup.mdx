...
- Authentication format for the returned API key.
...
- `apiKeyId`.
- `apiKey` and `apiKeyPrefix`.
- Billing status, free-credit allowance, and currency.
- `gettingStarted` summary.
- `setupChecklist`.

Store the raw `apiKey` immediately. It is shown once and is the bearer credential agents and Zapier use.
