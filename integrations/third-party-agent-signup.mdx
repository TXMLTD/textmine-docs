...
For the UK multitenant app, use `https://app.textmine.com` as the root base URL. The commonly used signup URLs are:

| URL | Purpose |
| --- | --- |
| `https://app.textmine.com/.well-known/textmine-agent.json` | Public discovery document. |
| `https://app.textmine.com/v3/third-party-agents/signup/availability` | Signup availability. |
| `https://app.textmine.com/v3/third-party-agents/signup/discovery` | Public API V3 signup discovery. |
| `https://app.textmine.com/v3/third-party-agents/signup/self-service` | Self-service signup request. |

Use the root base URL `https://app.textmine.com` when configuring clients. Do not use tenant names or tenant-specific aliases as selectable values for external users; the API key resolves the correct organization.
...
3. Create or select a [Vault workspace](/vault/overview), or pass `vault_id` when uploading.
