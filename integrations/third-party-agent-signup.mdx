---
title: "Agent self-service signup"
sidebarTitle: "Agent self-serve"
description: "Self-serve onboarding for third-party agents that need a TextMine organization, service-account API key, billing, Public API V3, and MCP access."
---

**Agent self-service signup** provisions a TextMine organization and service-account API credential for an external agent, automation platform, or MCP client that needs to call Public API V3 and the [TextMine MCP server](/integrations/textmine-mcp-server).
...
| Endpoint | Purpose |
| --- | --- |
| `/.well-known/textmine-agent.json` | Public machine-readable discovery document for agent self-service signup, billing, Public API V3, and MCP metadata. |
...
| URL | Purpose |
| --- | --- |
| `https://app.textmine.com/.well-known/textmine-agent.json` | Public agent discovery document. |
...
| `https://app.textmine.com/mcp` | Hosted TextMine MCP endpoint for the same environment. |
...
The same credential can call the TextMine MCP server:

```text
POST <TEXTMINE_MCP_URL>
...
3. Create or select a [Vault workspace](/vault/overview), or omit `vault_id` on the first upload so TextMine creates or reuses the default `Agent Vault` workspace for the API-key organization.
