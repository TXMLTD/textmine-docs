...
For the UK multitenant deployment, use `https://public-api.textmine.com` as the Public API base URL for discovery and signup. The hosted MCP endpoint remains `https://app.textmine.com/mcp`.
...
Use the Public API base URL `https://public-api.textmine.com` when configuring clients. Do not use `https://app.textmine.com`, tenant names, or tenant-specific aliases for Public API signup/discovery; the API key resolves the correct organization.
