...
| `search_accessible_vault_documents` | Searches documents and extracted tags across accessible Vaults. | Resolves source scopes before asking Vault or creating reports. |
...
| Capability | What it does | Notes |
| --- | --- | --- |
| `create_report` | Creates a report, table, comparison, summary, or analysis over selected documents. | Asynchronous; should honor resolved source scope. |
...
| `create_workflow` | Creates a reusable workflow using the workflow block catalog. | Requires approval; see [Workflow blocks](/workflows/blocks). |
| `run_workflow` | Runs an existing workflow or one-off workflow-backed action. | Asynchronous; followed by `wait_for_workflow_completion`. |
