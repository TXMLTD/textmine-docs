...
4. Upload a file:
   ...
   ```
   `vault_id` is optional. If the agent omits it, TextMine reuses the first accessible vault or creates the default `Agent Vault` workspace for that API-key organization before uploading the document.
